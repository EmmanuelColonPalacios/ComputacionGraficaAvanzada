...
rotBuzzLeftArm = 0.0, rotBuzzLeftCalf = 0.0, rotBuzzLeftFoot = 0.0, rotBuzzLeftForearm = 0.0, rotBuzzLeftHand = 0.0, rotLeftThigh = 0.0, rotLeftWing1 = 0.0, rotLeftWing2 = 0.0, 
rotBuzzRightArm = 0.0, rotBuzzRightCalf = 0.0, rotBuzzRightFoot = 0.0, rotBuzzRightForearm = 0.0, rotBuzzRightHand = 0.0, rotRightThigh = 0.0, rotRightWing1 = 0.0, rotRightWing2 = 0.0;
...
int stateDoor = 0;
float dorRotCount = 0.0, dorRotCount2;

float avance = 0.1f, avance2 = 0.2f;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	modelHeliRoar.loadModel("../models/Helicopter/Mi_24_heli_rear.obj");
	modelHeliRoar.setShader(&shaderMulLighting);
	//Mi_24_heli_rear.obj es la hélice trasera
...
bool processInput(bool continueApplication) {
	...
	if (enableCountSelected && glfwGetKey(window, GLFW_KEY_TAB) == GLFW_PRESS){
		...
		modelSelected++;
		if(modelSelected > 4) // Cambio para incluir a Buzz de 2 a 4 modelSelected > 2
			modelSelected = 0;
		if(modelSelected == 1)
...
void applicationLoop() {
	...
	float rotWheelsY = 0.0, rotWheelsY2 = 0.0;
	int numberAdvance = 0, numberAdvance2 = 0;
	int maxAdvance = 0.0, maxAdvance2 = 0.0;
	...
	while (psi) {
		...
		*******************************************/
		...
		glm::mat4 modelMatrixHeliRoar = glm::mat4(modelMatrixHeliChasis);
		modelMatrixHeliRoar = glm::translate(modelMatrixHeliRoar, glm::vec3(0.0, 2.10266, -5.63722));
		modelMatrixHeliRoar = glm::rotate(modelMatrixHeliRoar, rotHelHelY, glm::vec3(1,0,0));
		modelMatrixHeliRoar = glm::translate(modelMatrixHeliRoar, glm::vec3(-0.479976, -2.10266, 5.63722));
		modelHeliRoar.render(modelMatrixHeliRoar);
		...
		modelMatrixFrontWheelsLambo = glm::translate(modelMatrixFrontWheelsLambo, glm::vec3(0.0 , 0.377732, 1.39886));
		modelMatrixFrontWheelsLambo = glm::rotate(modelMatrixFrontWheelsLambo, rotWheelsY2, glm::vec3(0, -1, 0)); // giro vuelta
		modelMatrixFrontWheelsLambo = glm::rotate(modelMatrixFrontWheelsLambo, rotWheelsX2, glm::vec3(1, 0, 0)); // giro movimiento
		modelMatrixFrontWheelsLambo = glm::translate(modelMatrixFrontWheelsLambo, glm::vec3(0.0 , -0.377732, -1.39886));
		...
		modelMatrixRearWheelsLambo = glm::translate(modelMatrixRearWheelsLambo, glm::vec3(0.0, 0.377732, -1.56611));
		modelMatrixRearWheelsLambo = glm::rotate(modelMatrixRearWheelsLambo, rotWheelsY2, glm::vec3(0, -1, 0)); // giro vuelta
		...
		// ----> Desactivado :: glDisable(GL_CULL_FACE);
		...
		modelMatrixRightArm = glm::translate(modelMatrixRightArm, glm::vec3(-0.190291, 0.597859, -0.002768));
		...
		modelMatrixRightForeArm = glm::translate(modelMatrixRightForeArm, glm::vec3(-0.299587, 0.595169, -0.009158));
		...
		//modelMatrixHip = glm::scale(modelMatrixHip, glm::vec3(3.0));
		modelMatrixHip = glm::translate(modelMatrixHip, glm::vec3(0.000749, 0.018762, 0.440797));
		modelMatrixHip = glm::translate(modelMatrixHip, glm::vec3(-0.000749, -0.018762, -0.440797));
		...
		modelMatrixLeftThigh = glm::translate(modelMatrixLeftThigh, glm::vec3(0.058533, 0.392079, -0.0034));
		//modelMatrixLeftThigh = glm::rotate(modelMatrixLeftThigh, glm::radians(65.0f), glm::vec3(0, 0, 1));
		modelMatrixLeftThigh = glm::rotate(modelMatrixLeftThigh, rotLeftThigh, glm::vec3(1, 0, 0));
		modelMatrixLeftThigh = glm::translate(modelMatrixLeftThigh, glm::vec3(-0.058533, -0.392079, 0.0034));
		modelBuzzLeftThigh.render(modelMatrixLeftThigh);

		glm::mat4 modelMatrixLeftCalf = glm::mat4(modelMatrixLeftThigh);
		modelMatrixLeftCalf = glm::translate(modelMatrixLeftCalf, glm::vec3(0.068685, 0.217595, 0.003752));
		...
		modelMatrixLeftFoot = glm::translate(modelMatrixLeftFoot, glm::vec3(0.069464, 0.116051, 0.005957));
		...
		modelMatrixRightThigh = glm::translate(modelMatrixRightThigh, glm::vec3(-0.065245, 0.387596 , -0.020162));
		modelMatrixRightThigh = glm::rotate(modelMatrixRightThigh, rotRightThigh, glm::vec3(1, 0, 0));
		modelMatrixRightThigh = glm::translate(modelMatrixRightThigh, glm::vec3(0.065245, -0.387596 , 0.020162));
		modelBuzzRightThigh.render(modelMatrixRightThigh);

		glm::mat4 modelMatrixRightCalf = glm::mat4(modelMatrixRightThigh);
		modelMatrixRightCalf = glm::translate(modelMatrixRightCalf, glm::vec3(-0.057319, 0.212892, 0.002468));
		...
		modelMatrixRightFoot = glm::translate(modelMatrixRightFoot, glm::vec3(-0.075037, 0.112798, 0.003537));
		...
		glm::mat4 modelMatrixRightWing2 = glm::mat4(modelMatrixLeftWing1);
		modelBuzzRightWing2.render(modelMatrixRightWing2);


		// ----> Activado :: glEnable(GL_CULL_FACE);
		...
		else if(!record && keyFramesDartJoints.size() > 0){ //Para reproducir frames
		...
		if(record && modelSelected == 1){
			...
			matrixBuzzJoints.push_back(rotLeftWing1);
			matrixBuzzJoints.push_back(rotLeftWing2);
			matrixBuzzJoints.push_back(rotLeftThigh);
			...
			matrixBuzzJoints.push_back(rotRightWing1);
			matrixBuzzJoints.push_back(rotRightWing2);
			matrixBuzzJoints.push_back(rotRightThigh);
		...
		else if(!record && keyFramesBuzzJoints.size() > 0){ //Para reproducir frames
			...
			rotBuzzHead = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotBuzzTorso = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotBuzzHip = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);

			rotBuzzLeftArm = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotBuzzLeftForearm = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotBuzzLeftHand = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotBuzzLeftCalf = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotBuzzLeftFoot = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotLeftThigh = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotLeftWing1 = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotLeftWing2 = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);

			rotBuzzRightArm = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotBuzzRightForearm = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotBuzzRightHand = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotBuzzRightCalf = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotBuzzRightFoot = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotRightThigh = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotRightWing1 = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
			rotRightWing2 = interpolate(keyFramesBuzzJoints, indexFrameBuzzJoints, indexFrameBuzzJointsNext, 
			0, interpolationBuzzJoints);
		...
		if(record && modelSelected == 4){
			...
			if(indexFrameBuzzNext > keyFramesBuzz.size() - 1)
				...
				modelMatrixBuzz = interpolate(keyFramesBuzz, indexFrameBuzz, indexFrameBuzzNext, 
				0, interpolationBuzz); // chequea esto memi
		...
		switch (state2)
{
    case 0:
        ...
        break;
    case 1:
        modelMatrixLambo = glm::translate(modelMatrixLambo, glm::vec3(0.0f, 0.0f, avance2));
        advanceCount2 += avance2;
        rotWheelsX2 += 0.05;
        rotWheelsY2 -= 0.02;
        if (rotWheelsY2 < 0.0)
            rotWheelsY2 = 0.0;
        if (advanceCount2 > maxAdvance2) {
            ...
            numberAdvance2++;
            if (numberAdvance2 == 5)
                state2 = 3; // Nuevo estado para abrir la puerta
            else
                state2 = 2;
        }
        break;
    case 2:
        ...
        modelMatrixLambo = glm::rotate(modelMatrixLambo, glm::radians(-giroLambo),
            glm::vec3(0, 1, 0));
        ...
        break;
    case 3: // Nuevo estado para abrir la puerta
        dorRotCount += 0.6;
        if (dorRotCount > 75.0)
            state2 = 4;
        break;
    case 4: // Nuevo estado para abrir la puerta
        dorRotCount -= 0.6;
        if (dorRotCount < 0)
            state2 = 3; // Volver al estado inicial
        break;
    default:
        break;
}
